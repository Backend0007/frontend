...
import { LUCIDE_ICONS, LucideAngularModule, LucideIconProvider, Route, User, icons } from 'lucide-angular';
...
export class TopbarComponent {
 ...
 messageErreur!: any;
  ...
  ngOnInit(): void {
    ...
    this.loaderUser();
    // check connexion from backend

    this.apiRestDisponibility.checkConnexion()
        .subscribe(Response => {
          if(Response == 900){
            console.log(Response);
            this.messageErreur = 'Serveur en ligne';
            this.errorBin = true;
          } else {
            this.messageErreur = 'Serveur éteint';
            this.errorBin = false;
          }
        },
        error => {
          switch(this.errorHandlerService.checkDisponibility(error)){
            case 0:
              this.errorBin = false;
              this.messageErreur = 'Serveur éteint ou injoignable!';
            break;
            case 404:
              this.errorBin = false;
              this.messageErreur = 'Ressource non trouv\é!';
              break;
            case 500:
              this.errorBin = false;
              this.messageErreur = 'Erreur du serveur interne!';
              break;
            case 10:
              this.errorBin = false;
              this.messageErreur = 'Serveur Eteint!';
              break;
          }
        });
